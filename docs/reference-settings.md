...
**Font**: Click this to pick the interface font from a menu of the fonts installed on your system. There's also a **Default** option and a **Default Unicode** option (handy if you need broader character coverage). It's usually best to leave this on Default. (Default: Default)
...
**Panels open side-by-side**: When enabled, you can open two or more panels next to each other; when disabled, panels stack on top of one another instead. Leaving this on suits most setups. (Default: disabled)
...
> 💡 **Tip:** Another option is to turn this off and use keyboard shortcuts to open and close the panels instead: F11 for the Actions panel and B for the Browser.
...
**Show track outputs**: Adds a speaker icon to the right of each track's Mute and Solo buttons; click it to set the track's hardware or virtual output. Even with this off, the output assignment is still reachable from the plugins section whenever a track is tall enough. This option just makes it available on narrow tracks too, so leaving it off is fine for most setups. (Default: enabled)
...
> 💡 **Tip:** A common approach is to keep the buffer small while tracking so you can monitor without noticeable delay, then bump it up for mixing once the CPU is busy with plugins.
...
The **Edit Control Mappings...** button opens a dedicated window listing every mapping on a custom surface. To learn a control, click a row's "learn" side and wiggle the physical knob or fader, and Waveform picks up the incoming message. The other half of each row lets you choose which Waveform function the control drives. Double-click a row to edit it manually, and press Delete to remove a mapping.
...
- *Drag to draw marked region* gives you the I-beam range cursor, an editing mode many users prefer. Hold Opt / Alt while dragging to select clips instead.
...
**Allow inputs to appear on multiple tracks**: Lets one input (a MIDI device or audio input) be assigned to more than one track. It's a specialised need, and most users leave it off. Only appears if your edition supports multi-input. (Default: off)
...
- **Three keys is the limit.** Each command holds at most three combinations. Once a row is full, the **+** button won't add a fourth, so remove one first.
...
If the login is cancelled or something goes wrong fetching your licenses, Waveform shows a warning explaining the problem and leaves your existing licenses untouched. You can try again.
