...
> 💡 **Tip:** If you use tags heavily, keep the tags bar open and give it some vertical space. It becomes a handy way to cross-filter results. For example, try selecting "Packs" in the sidebar and then clicking a "Bass" tag.
...
On the left side of the Browser is a narrow sidebar divided into four sections. Only one item can be selected across all four sections at a time, so clicking an item in one section automatically deselects items in the others.
...
> 💡 **Tip:** Keeping Auto-play and Loop on suits most workflows. It lets you audition content just by clicking through the results list, which is much faster than manually hitting Play each time.
...
2. Open the track properties and find the **Tags** field.
...
Next, you might want to explore how presets and racks work in more detail.
